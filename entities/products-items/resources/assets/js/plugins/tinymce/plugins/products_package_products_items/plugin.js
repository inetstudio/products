...
window.tinymce.PluginManager.add('products_package_products_items', function(editor) {
    let widgetData = {
        widget: {
            events: {
                widgetSaved: function(model) {
                    editor.execCommand(
                        'mceReplaceContent',
                        false,
                        '<img class="content-widget" data-type="products.list" data-id="' + model.id + '" alt="Виджет-подборка" />',
                    );
                }
            }
        }
    ...
    function loadWidget() {
        let component = window.Admin.vue.helpers.getVueComponent('products_package_products_items', 'ProductsPackageProductsItemsList');

        component.$data.model.id = widgetData.model.id;
    }

    editor.addButton('add_products_items_list_widget', {
        ...
        onclick: function() {
            ...
            let isProductsItemsList = /<img class="content-widget".+data-type="products.list".+>/g.test(content);

            if (content === '' || isProductsItemsList) {
                widgetData.model = {
                    id: parseInt($(content).attr('data-id')) || 0,
                };

                window.Admin.vue.helpers.initComponent('products_package_products_items', 'ProductsPackageProductsItemsList', widgetData);

                window.waitForElement('#add_products_items_list_widget_modal', function() {
                    loadWidget();

                    $('#add_products_items_list_widget_modal').modal();
                });
            } else {
                Swal.fire({
                    title: 'Ошибка',
                    text: 'Необходимо выбрать виджет-подборку',
                    icon: 'error',
                });
